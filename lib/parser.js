...
export function processSheetData(rows) {
  ...
  for (const record of records) {
    ...
    // Check for Day: "Day X" usually implies a new day. 
    // "Training/Workout" (אימון) might be a day header OR just a category description for an exercise.
    // If it has sets/reps data (col 2 or 3), it's likely an exercise line, NOT a new day header.
    const isDayRow = (record[0] && record[0].includes('יום'));
    const isWorkoutHeader = (record[0] && record[0].includes('אימון') && !record[2] && !record[3]);

    if (isDayRow || isWorkoutHeader) {
      // If no week exists yet, create a default one
      if (!currentWeek) {
        currentWeek = { name: 'שבוע 1', dateRange: 'General', days: [] };
      }
